...
struct choose_tests {
  void operator()(const auto &params) {
    sketch_impl_type sketch_impl =
        krowkee::util::get_sketch_impl_type(params.sketch_impl().c_str());
    cmap_impl_type cmap_impl =
        krowkee::util::get_cmap_impl_type(params.cmap_impl().c_str());
    if (sketch_impl == sketch_impl_type::cst) {
      perform_tests<dense::SparseJLT<RangeSize, ReplicationCount>,
                    make_ptr_functor>(params);
    } else if (sketch_impl == sketch_impl_type::sparse_cst) {
      if (cmap_impl == cmap_impl_type::std) {
        perform_tests<sparse::map::SparseJLT<RangeSize, ReplicationCount>,
                      make_ptr_functor>(params);
#if __has_include(<boost/container/flat_map.hpp>)
      } else if (cmap_impl == cmap_impl_type::boost) {
        perform_tests<sparse::flatmap::SparseJLT<RangeSize, ReplicationCount>,
                      make_ptr_functor>(params);
#endif
      }
    } else if (sketch_impl == sketch_impl_type::promotable_cst) {
      if (cmap_impl == cmap_impl_type::std) {
    ...
    } else if (sketch_impl == sketch_impl_type::fwht) {
...
struct parameters : public krowkee::test::sketch::parameters {
  ...
  krowkee::parameter<std::string> sketch_impl;
  krowkee::parameter<std::string> cmap_impl;

  parameters()
      : base_type(),
        sketch_impl("sketch_impl", "Sketch type", 't', true, "cst"),
        cmap_impl("cmap_impl", "Compacting map type", 'm', true, "std") {
    _params.push_back(&sketch_impl);
    _params.push_back(&cmap_impl);
  }
};
